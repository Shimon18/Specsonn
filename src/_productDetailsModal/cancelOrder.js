...
export default class cancelOrderModal extends React.Component {
    ...
    constructor(props){
        ...
        this.state = {
            ...
            reason : 'Shipping Address Undeliverable',
            comments : null,                        
            errors:{
                comments : '',
                reason : ''
    ...
    clickTOReportCancel = (event) =>
    {
        ...
        else
        {
            ...
            PostData(global.userOrderCancel, postData,'POST').then((result) => {
                ...
                <form>
                    <div className="form-group iris_add_address_form_element">
                        <div className="iris_add_address_form_element_icon">
                            <img src={global.BASE_URL + "assets/img/iris_icon_1.png"} alt="" title=""/></div>
                        <div className="iris_add_address_form_element_txt"><span>{_prlTxt.orderpage.reason}</span>
                            <select name="reason" id="reason" onChange={this.handleChange}>
                                <option disabled>{_prlTxt.orderpage.slreason}</option>
                                <option value="Shipping Address Undeliverable" selected>{_prlTxt.orderpage.rs_addun}</option>
                                <option value="Customer Exchange">{_prlTxt.orderpage.rs_cust}</option>
                                <option value="Product Defect">{_prlTxt.orderpage.rs_prdef}</option>
                                <option value="Product Not Look Like as Shown">{_prlTxt.orderpage.rs_prolooksme}</option>
                            </select>
                        </div>
                    ...
                    <div className="form-group">
                        <button type="button" className="iris_addaddress_btn" onClick={this.clickTOReportCancel}>{_prlTxt.submit} &gt;</button>
                    </div>
                </form>
            </div>
            )
