...
class genderSection extends Component {
    ...
    renderView(handle,filterParam,know){
        ...
        let _urlData = global.genderListSync+'?category_Id=1&country_Id='+global.country_Ids+'&language_Id='+global.language_Ids+'&filterId='+_filterId+filterParam+'&limit='+_limit+',6';
        ...
        .then(res => {
            ...
            if(res.dataCount == 0){
                ...
                let _urlSorting = global.sortingListSync+'?category_Id='+_categoryId+'&country_Id='+global.country_Ids+'&language_Id='+global.language_Ids+'&typTxt='+_typTxt+'&catsTxt='+_catsTxt+_filGen+'&master='+_filterArrys.master+filterParam;
                ...
                { 
                    _listing.forEach((items, index) => {
                        //console.log(items.product_Name_Display+'-----'+index);
                ...
                for(let _k = 0; _k < _extraData_Imgs.length; _k++)
                {
                    ...
                    {                    
                        if(_extraData_Colrs[_k].color_type === 'HEXA')
                        {
                            ...
                            _ColorSmallBox.push(<a key={_proids+_k} href={global.BASE_URL+item.menuUrl+'/'+_extraData_Colrs[_k].product_Url} onMouseOver={this.mouseOver.bind(this, i, _k)}><span className={_actCls+" colorIcons"} id={_proids}><span className="colorIcons_colorTop" style={{'background':_spnColA}}><span className="colorIcons_colorBottom" style={{'background':_spnColB}}></span></span></span></a>);
                        }
                        else
                        {
                            _ColorSmallBox.push(<a key={_proids+_k} href={global.BASE_URL+item.menuUrl+'/'+_extraData_Colrs[_k].product_Url} onMouseOver={this.mouseOver.bind(this, i, _k)}><img id={_proids} className={_actCls+" colorIcons"} src={item.colorPath+_extraData_Colrs[_k].colors_Hexa} alt={_extraData_Colrs[_k].colors_Name} style={{width:'15px'}} ></img></a>)
                        } 
                    }                   
                }                
    ...
    handleFilterboxChange(e,i,t,event){        
        ...
        filterParam = '&frameSize_id='+_frameSize+'&frameShape_Id='+_frameShape+'&frametype_Id='+_frameType+'&colors_Id='+_frameColor+'&brand_Id='+_frameBrand+'&material_id='+_frameMaterial+'&priceBetween='+this.state.onSort+'&priceRange='+_framePrice;
    ...
    handleSortboxChange(event){
        ...
        filterParam = '&frameSize_id='+_frameSize+'&frameShape_Id='+_frameShape+'&frametype_Id='+_frameType+'&colors_Id='+_frameColor+'&brand_Id='+_frameBrand+'&material_id='+_frameMaterial+'&priceBetween='+event.target.value+'&priceRange='+_framePrice;

        this.renderView(handle,filterParam,'F');
        this.setState({filterHeader:filterHeader,onSort:event.target.value,onLimit:0,_listing:[]});
        return;      
    }
    ...
    removeFilterHeader(item){
        ...
        this.renderView(handle,filterParam,'F');
        this.setState({filterHeader:filterHeader,onLimit:0,_listing:[]});        
        return;
    }
    ...
    sortingArea(){
        if(this.state.isLoaded === true)
        {
            ...
            {
                if(_oldFrameType.length == 0){
                    _oldFrameType = this.state.frameType;
                }
